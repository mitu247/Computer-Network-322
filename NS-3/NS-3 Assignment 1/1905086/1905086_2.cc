...
#define WIRE_DATA_RATE "2Mbps"
#define WIRE_DELAY "1ms"
#define SIMULATION_TIME 9.0
uint32_t recPacketsCnt, sentPacketsCnt;
std::string path = "scratch/mobile";
...
CalculateMetric(ApplicationContainer sinks, int plotCat) {
    ...
    if (plotCat == 1) {
        ...
        // std::cout << "val: " << val << ", no of apps: " << sinks.GetN() << std::endl;
    }
    else if (plotCat == 2) {
...
main(int argc, char* argv[])
{
    ...
    int plotCat = 1; // 1 -> throughput, 2 -> packet delivery ratio
    ...
    mobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel","Bounds",RectangleValue(Rectangle(-50, 50, -50, 50)), "Speed", StringValue ("ns3::ConstantRandomVariable[Constant="+std::to_string(speed)+"]"));
    ...
    if (plotCat == 1 and plotNo == 4) Plot("SpeedVSThroughput.dat", speed, metric);
    ...
    if (plotCat == 2 and plotNo == 4) Plot("SpeedVSPacketDeliveryRatio.dat", speed, metric);
